...
def check_freq(fname):
    ...
    # 파일이름 앞에 알파벳 2개 시작 파일 체크 예) en-1.txt
...
def load_files(path):
    ...
    if DEBUG:
        print('load_files() => file_list = {}'.format(file_list))
...
# (1) 데이터 준비하기 ----------------------------------------
data = load_files("../../DATA/LANG/train/*.txt")  # 결과 dictionary 타입 리턴
test = load_files("../../DATA/LANG/test/*.txt")   # 결과 dictionary 타입 리턴

# JSON으로 결과 저장하기
with open("../../DATA/LANG/freq.json", "w", encoding="utf-8") as fp:
    json.dump([data, test], fp)     # json 문자열 데이터로 변환
...
# 학습 데이터 저장하기
joblib.dump(clf, "../../DATA/LANG/freq.pkl")  # 검출/분류기 파일 저장
print("ok")
