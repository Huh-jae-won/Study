...
if KIND == 1:
    for i in plt.style.available:
        print(i)    # 지원하는 style 리스트 출력
    plt.title('라인 플로트')
    plt.plot(['a','c','e'],[3,5,7], 'v--')
    plt.plot(['a','b','c','d'], [1,2,3,4],'o-')       # 데이터 plot(x축 데이터, y축 데이터)
    plt.xlabel('value')
    plt.ylabel('data')
    ...
    # 수직으로 하려면 plt.bar(x,y,width=0.1, color="blue")
    plt.show()
    ...
    plt.figure(figsize=(15,10))         # figsize는 inch사이즈
